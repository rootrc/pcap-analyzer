...
namespace net::udp {
    size_t parse(BufferView& buf, Header& header, uint64_t pseudoHeaderSum, Endian endian);
    ...
    size_t parse(BufferView& buf, Header& header, uint64_t pseudoHeaderSum, Endian endian) {
        ...
        if (toHost16(header.length, endian) < HEADER_LEN || toHost16(header.length, endian) > buf.length()) {
            return 0;
        }
